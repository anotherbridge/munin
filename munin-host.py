...
def process_elements(
    ...
    while c < len(elements):
        ...
        if debug:
            print(
                "i: %d - Elements in queue: %d Element in check: %s"
                % (c, len(elements), element)
            )
        ...
        for iwl in IP_WHITE_LIST:
            if iwl == value:
                print_highlighted(
                    "IP white-listed - skipping this host SYSTEM: %s" % value
                )
                status = "skipped"
        ...
        while not success:
            try:
                ...
                response = urllib.request.urlopen(
                    "%s?%s" % (URLS[cat], urllib.parse.urlencode(parameters)),
                    context=ctx,
                ).read()
        ...
        if response_dict.get("response_code") > 0:
            ...
            if "resolutions" in response_dict:
                ...
                for i, res in enumerate(resolution_list):
                    resolutions.append(
                        {
                            "target": res[RES_TARGETS[cat]],
                            "last_resolved": res["last_resolved"],
                        }
                    )
                    if i < max_items:
                        print_highlighted(
                            "HOST: {0} LAST_RESOLVED: {1}".format(
                                res[RES_TARGETS[cat]], res["last_resolved"]
                            )
                        )
                    else:
            ...
            if "detected_urls" in response_dict:
                ...
                for i, url in enumerate(detected_urls):
                    ...
                    if i < max_items and args.download:
                        print_highlighted(
                            "URL: {0} POSITIVES: {1} TOTAL: {2}".format(
                                url["url"], positives_url, total_url
                            )
                        )
                    else:
            ...
            if "detected_communicating_samples" in response_dict:
                ...
                for i, sample in enumerate(samples_list):
                    ...
                    if i < max_items:
                        print_highlighted(
                            "SAMPLE: {0} POSITIVES: {1} TOTAL: {2} "
                            "DATE: {3}".format(
                                sha256, positives_sample, total_sample, date
                            )
                        )
                    else:
...
def download_url(host_id, url):
    ...
    if c.getinfo(c.RESPONSE_CODE) == 200:
        ...
        try:
            print_highlighted(
                "[i] Response CODE: %s MIME_TYPE: %s SIZE: %s"
                % (
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Virustotal Online Checker (IP/Domain)"
    )
    parser.add_argument(
        "-f",
    ...
    parser.add_argument(
        "-m",
        help="Maximum number of items (urls, hosts, samples) to show",
        metavar="max-items",
        default=10,
    )
    parser.add_argument(
        "-c",
        help="Name of the cache database file (default: vt-hosts-db.json)",
        metavar="cache-db",
    ...
    parser.add_argument(
        "--nocache",
        action="store_true",
        help="Do not use the load the cache db (vt-check-cache.pkl)",
        default=False,
    )
    parser.add_argument(
        "--nocsv",
    ...
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="Process the resolved IPs as well",
        default=False,
    )
    parser.add_argument(
        "--download",
        action="store_true",
        help="Try to download the URLs (directories with host/ip names)",
        default=False,
    )
    parser.add_argument(
        "-d",
        help="Store the downloads to the given directory",
    ...
    parser.add_argument(
        "--dups",
        action="store_true",
        help="Do not skip duplicate hashes",
        default=False,
    )
    parser.add_argument(
        "--noresolve",
        action="store_true",
        help="Do not perform DNS resolve test on found domain " "names",
        default=False,
    )
    parser.add_argument(
        "--ping",
        action="store_true",
        help="Perform ping check on IPs (speeds up process if many "
    ...
    if not args.nocache:
        ...
        if success:
            ...
            print(
                "[+] Analyzed IPs/domains will be written to cache database: {0}".format(
                    args.c
                )
            )
        print(
            "[+] You can always interrupt the scan by pressing CTRL+C without loosing the scan state"
        )
