...
def printResult(info, count, total):
    ...
    if "total" in info:
        ...
        printHighlighted(
            "TYPE: {1} SIZE: {2} FILENAMES: {0}".format(
                removeNonAsciiDrop(info["filenames"]),
                info["filetype"],
                info["filesize"],
            )
        )
        ...
        printPeInfo(info)
        printHighlighted(
            "FIRST: {0} LAST: {1} SUBMISSIONS: {5} REPUTATION: {6}\nCOMMENTS: {2} USERS: {3} TAGS: {4}".format(
...
def printHighlighted(line, hl_color=Back.WHITE, tag_color=False):
    ...
    if tag_color:
        ...
        line = colorer.sub(
            Fore.BLACK + Back.YELLOW + r"\1" + Style.RESET_ALL + "", line
        )
    # Extras
    colorer = re.compile("(\[!\])", re.VERBOSE)
    line = colorer.sub(
        Fore.BLACK + Back.LIGHTMAGENTA_EX + r"\1" + Style.RESET_ALL + "", line
    )
    # Add line breaks
...
def printSeparator(count, total, color, rating):
    ...
    print(
        " {0} / {1} > {2}".format(count + 1, total, rating.title()).ljust(80)
        + Style.RESET_ALL
    )
