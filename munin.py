...
try:
    ...
    print(
        "ERROR: Module 'PyMISP' not found (this feature will be deactivated: MISP queries)"
    )
...
try:
    ...
    traceback.print_exc()
    print(
        "ERROR: Module 'flask' or 'flask_caching' not found (try to fix this with 'pip3 install flask flask_caching'"
    )
...
HYBRID_ANALYSIS_DOWNLOAD_URL = (
    "https://www.hybrid-analysis.com/api/v2/overview/%s/sample"
)
...
INTEZER_NON_QUOTA_URL = (
    "https://analyze.intezer.com/api/v1-2/code-items/%s/latest-public-analysis"
)
    ...
    if args.nocache or not cache_result:
        ...
        if not config.has_option("VIRUSTOTAL", "SKIP_SERVICES"):
            ...
            if not "Hashlookup" in excludes:
                hashlookup_info = getHashlookup(
                    info["md5"], info["sha1"], info["sha256"]
                )
    ...
    for i, line in enumerate(lines):
        ...
        if not info or (
            ...
            if args.comment and info["sha256"] != "-":
                munin_vt.commentVTSample(
                    info["sha256"], "%s %s" % (args.p, info["comment"])
                )
    ...
    for c, m_url in enumerate(MISP_URLS, start=0):
        ...
        try:
            ...
            misp = pymisp.PyMISP(
                ...
                cert=None,
                auth=None,
                tool="Munin : Online hash checker",
            )
            if args.debug:
                print("[D] Query: values=%s" % hash)
            result = misp.search(
                "attributes", type_attribute=fetchHash(hash)[1], value=hash
            )
            ...
            if result["Attribute"]:
                ...
                if args.debug:
                    print(
                        "[D] Dump Attribute : "
                        + json.dumps(result["Attribute"], indent=2)
                    )
...
def getValhalla(sha256):
    ...
    try:
        ...
        response = requests.post(
            VALHALLA_URL, data=data, proxies=connections.PROXY, timeout=15
        )
        if args.debug:
...
def getTotalHashInfo(sha1):
    ...
    try:
        ...
        if (
            ...
            and "Sorry something went wrong" not in response.content
        ):
...
def getURLhaus(md5, sha256):
    ...
    try:
        if sha256:
            ...
            data = {"md5_hash": md5}
        response = requests.post(
            URL_HAUS_URL, data=data, timeout=3, proxies=connections.PROXY
        )
        # print("Response: '%s'" % response.json())
        res = response.json()
...
def getCAPE(md5, sha1, sha256):
    ...
    try:
        if sha256 != "-":
            response = requests.get(
                URL_CAPE_SHA256 % sha256, timeout=3, proxies=connections.PROXY
            )
        elif sha1 != "-":
            response = requests.get(
                URL_CAPE_SHA1 % sha1, timeout=3, proxies=connections.PROXY
            )
        elif md5 != "-":
            response = requests.get(
                URL_CAPE_MD5 % md5, timeout=3, proxies=connections.PROXY
            )
...
def getVirusBayInfo(hash):
    ...
    try:
        ...
        if response["search"] != []:
            ...
            info["vb_tags"] = tags
            info["vb_link"] = (
                "https://beta.virusbay.io/sample/browse/%s"
                % response["search"][0]["md5"]
            )
    except Exception as e:
        if args.debug:
...
def peChecks(info, infos):
    ...
    if imphash_count > 1:
        printHighlighted(
            "[!] Imphash - appeared %d times in this batch %s"
        ...
        for s in infos:
            if "signer" in s and "signer" in info:
                if (
                    ...
                    and not any(s in info["signer"] for s in SIGNER_WHITELIST)
                ):
                    signer_count += 1
        if signer_count > 1:
            printHighlighted(
                "[!] Signer - appeared %d times in this batch %s"
        ...
        if "misp_available" in info:
            if info["misp_available"]:
                for e in info["misp_info"]:
                    printHighlighted(
                        "[!] MISP event found EVENT_ID: {0} EVENT_INFO: {1} URL: {2}".format(
    ...
    try:
        ...
        if "malshare_available" in info:
            if info["malshare_available"]:
                printHighlighted(
                    "[!] Sample is available on malshare.com URL: {0}{1}".format(
    ...
    try:
        ...
        if "malware_bazar_available" in info:
            if info["malware_bazar_available"]:
                printHighlighted(
                    "[!] Sample is available on Malware Bazar URL: {0}".format(
    ...
    try:
        ...
        if "hybrid_available" in info:
            if info["hybrid_available"]:
                printHighlighted(
                    "[!] Sample is on hybrid-analysis.com SCORE: {0} URL: {1}/{2}".format(
    ...
    try:
        ...
        if "intezer_available" in info:
            if info["intezer_available"]:
                printHighlighted(
                    "[!] Sample is on Intezer URL: {0}".format(
                        (INTEZER_ANALYSIS_URL % info["sha256"])
                    )
                )
    except KeyError as e:
        if args.debug:
    ...
    try:
        ...
        if "urlhaus_available" in info:
            if info["urlhaus_available"]:
                printHighlighted(
                    "[!] Sample on URLHaus URL: %s" % info["urlhaus_download"]
                )
                printHighlighted(
                    "[!] URLHaus info TYPE: %s FIRST_SEEN: %s LAST_SEEN: %s URL_COUNT: %s"
                    % (
                        info["urlhaus_type"],
                        info["urlhaus_first"],
                        info["urlhaus_last"],
                ...
                for url in info["urlhaus_urls"]:
                    printHighlighted(
                        "[!] URLHaus STATUS: %s URL: %s"
                        % (url["url_status"], url["url"])
                    )
                    c += 1
        ...
        if "cape_available" in info:
            if info["cape_available"]:
                for report_id in info["cape_reports"]:
                    printHighlighted(
                        "[!] Sample on CAPE sandbox URL: https://capesandbox.com/analysis/%s/"
                        % report_id
                    )
    except KeyError as e:
        if args.debug:
    ...
    try:
        ...
        if info["valhalla_match"]:
            for m in info["valhalla_matches"]:
                ...
                if "DEMO" in m["tags"]:
                    feed = (
                        "public rule LINK: https://github.com/Neo23x0/signature-base/search?q=%s"
                        % m["rulename"]
                    )
                printHighlighted(
        ...
        if "hashlookup_available" in info:
            if info["hashlookup_available"]:
                for h in info["hashlookup_info"]:
                    printHighlighted(
                        "[!] Known File, details available on {} Hashlookup: {}".format(
                            h["hashlookup_source"], h["source_url"]
                        )
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vh",
        help="Query Valhalla for hashes by keyword, tags, YARA rule name, Mitre ATT&CK software (e.g. S0154), technique (e.g. T1023) or threat group (e.g. G0049)",
        metavar="search-string",
        default="",
    )
    parser.add_argument(
        "--vhrule",
        help="Query Valhalla for hashes via rules by keyword, tags, YARA rule name, Mitre ATT&CK software (e.g. S0154), technique (e.g. T1023) or threat group (e.g. G0049)",
        metavar="search-string",
        default="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vtwaitquota",
        action="store_true",
        help="Do not continue if VT quota is exceeded but wait for the next day",
        default=False,
    )
    parser.add_argument(
        "--vtminav",
        help='Minimum number of AV matches to query hash info from VT"',
        metavar="min-matches",
        default=0,
    )
    parser.add_argument(
        "--limit",
        help="Exit after handling this much new hashes in batch mode (cache ignored).",
        metavar="hash-limit",
        default=0,
    )
    parser.add_argument(
        "--vhmaxage",
        help="Maximum age of sample on Valhalla to process",
        metavar="days",
        default=0,
    )
    parser.add_argument(
        "-c",
        help="Name of the cache database file (default: vt-hash-db.json)",
        metavar="cache-db",
        default="vt-hash-db.json",
    )
    parser.add_argument(
        "-i",
        help="Name of the ini file that holds the API keys",
        metavar="ini-file",
    ...
    parser.add_argument(
        "--comment",
        action="store_true",
        help="Posts a comment for the analysed hash which contains "
        "the comment from the log line",
        default=False,
    )
    parser.add_argument(
        "-p",
        help="Virustotal comment prefix",
        metavar="vt-comment-prefix",
    ...
    parser.add_argument(
        "--download",
        action="store_true",
        help="Enables Sample Download from Hybrid Analysis. SHA256 of sample needed.",
        default=False,
    )
    parser.add_argument(
        "-d",
        help="Output Path for Sample Download from Hybrid Analysis. Folder must exist",
    ...
    parser.add_argument(
        "--nocache",
        action="store_true",
        help="Do not use cache database file",
        default=False,
    )
    parser.add_argument(
        "--nocsv",
        action="store_true",
        help="Do not write a CSV with the results",
        default=False,
    )
    parser.add_argument(
        "--verifycert",
    ...
    parser.add_argument(
        "--cli",
        action="store_true",
        help="Run Munin in command line interface mode",
        default=False,
    )
    parser.add_argument(
        "--rescan",
        action="store_true",
        help="Trigger a rescan of each analyzed file",
        default=False,
    )
    parser.add_argument(
    ...
    try:
        ...
        if config.has_section("VIRUSTOTAL"):
            try:
                VIRUSTOTAL_MAX_QUERY_SIZE = ast.literal_eval(
                    config.get("VIRUSTOTAL", "VIRUSTOTAL_MAX_QUERY_SIZE")
                )
                ...
                print(
                    "[E] Your config misses some key in the VIRUSTOTAL config - check the new munin.ini template and adapt it to your "
                    "config to avoid this error.",
                    e,
                )
        ...
        if config.has_section("VALHALLA"):
            try:
                VALHALLA_MAX_QUERY_SIZE = ast.literal_eval(
        ...
        if config.has_section("HASHLOOKUP"):
            has_hashlookup = True
            try:
                HASHLOOKUP_URLS = ast.literal_eval(
                    config.get("HASHLOOKUP", "HASHLOOKUP_URLS")
                )
                HASHLOOKUP_AUTH_KEYS = ast.literal_eval(
                    config.get("HASHLOOKUP", "HASHLOOKUP_AUTH_KEYS")
                )
                HASHLOOKUP_HANDLES = ast.literal_eval(
                    config.get("HASHLOOKUP", "HASHLOOKUP_HANDLES")
                )
            except Exception as e:
                print(
                    "[E] Your config misses some key in the HASHLOOKUP config - check the new munin.ini template and adapt it to your "
                    "config to avoid this error."
                )
                has_hashlookup = False
        ...
        if config.has_section("MISP"):
            ...
            try:
                ...
                if args.debug:
                    traceback.print_exc()
                print(
                    "[E] Since munin v0.13.0 you're able to define multiple MISP instances in config. The new .ini "
        ...
        traceback.print_exc()
        print(
            "[E] Config file '%s' not found or missing field - check the current munin.ini template if fields have "
            "changed or add the missing field manually" % args.i
        )
        sys.exit(1)

    # Check API Key
    if munin_vt.VT_PUBLIC_API_KEY == "" or not re.match(
        r"[a-fA-F0-9]{64}", munin_vt.VT_PUBLIC_API_KEY
    ):
    ...
    if not args.nocache:
        ...
        if success:
            print(
                "[+] {0} cache entries read from cache database: {1}".format(
                    len(cache), args.c
                )
            )
        else:
            ...
            print(
                "[+] Analyzed hashes will be written to cache database: {0}".format(
                    args.c
                )
            )
        print(
    ...
    if args.cli:
        ...
        print("")
        print(
            "Paste your content into the command line window and then press CTRL+D to process the pasted content."
        )
        print(
            "Make sure that your last content line has a line break at its end (press ENTER before CTRL+D)."
        )
    ...
    if args.web:
        if "flask" in deactivated_features:
            print(
                "[E] Flask module has not been loaded. Try to install it with 'pip3 install flask' before using "
                "this feature"
            )
            sys.exit(1)
        print("")
    ...
    if args.vh:
        ...
        try:
            ...
            for match in matches:
                ...
                if len(lines) > VALHALLA_MAX_QUERY_SIZE or len(lines) >= int(
                    args.limit
                ):
                    break
        except Exception as e:
    ...
    if (
        ...
        print(
            "[E] Use at least one of the options -f file, -s directory, --vh query, --web or --cli"
        )
        sys.exit(1)
